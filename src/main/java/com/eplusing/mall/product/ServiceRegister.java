...
public class ServiceRegister implements ApplicationRunner{
	...
	public void run(ApplicationArguments args) throws Exception {
		...
		CuratorFramework client = CuratorFrameworkFactory.newClient(zkAddress, new RetryOneTime(3000));
		client.start();
		client.blockUntilConnected();
		
		ServiceInstance<Object> instance = ServiceInstance.builder().name("product").address("192.168.1.103").port(9090).build();
